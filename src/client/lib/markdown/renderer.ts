...
md.renderer.rules.tabs_open = (tokens, index) => {
    const { id, titles } = tokens[index]!.meta as {
        id: string;
        titles: string[];
    };
    const buttons = titles
        .map((title, tabIndex) => `<button type="button" role="tab" id="${id}-tab-${tabIndex}" aria-controls="${id}-panel-${tabIndex}" aria-selected="${tabIndex === 0 ? 'true' : 'false'}" tabindex="${tabIndex === 0 ? '0' : '-1'}" data-tab-button="${tabIndex}">${escapeHtml(title)}</button>`)
        .join('');
...
md.renderer.rules.tab_panel_open = (tokens, index) => {
    const { id, tabIndex } = tokens[index]!.meta as {
        id: string;
        tabIndex: number;
    };
    return `<section class="tab-panel" role="tabpanel" id="${id}-panel-${tabIndex}" aria-labelledby="${id}-tab-${tabIndex}" data-tab-panel="${tabIndex}"${tabIndex === 0 ? '' : ' hidden'}>`;
};
...
function findContainerEnd(state: {
    ...
    for (let line = startLine + 1; line < endLine; line++) {
        ...
        if (fence)
            continue;
        if (new RegExp(`^:{${markerLength},}\\s+(?:details|tabs)\\b`).test(text))
            depth++;
